...
cp ../results_HybPhaser_allGenes/output_HybPhaser_allGenes/03_sequence_lists_strict/loci_consensus/${gene}*.fasta .
...
#Use MAFFT to align.
mafft ${gene}_subset.fasta > ${gene}_mafft_preliminary_strict.fasta
cp ${gene}_mafft_preliminary_strict.fasta ../results_mafft_preliminary_strict/
...
trimal -in ${gene}_mafft_preliminary_strict.fasta -out ${gene}_trimal_preliminary_strict.fasta -resoverlap 0.75 -seqoverlap 0.90 -gt 0.90

#Save results from TrimAl.
cp ${gene}_trimal_preliminary_strict.fasta ../results_trimal_preliminary_strict/

#Mask possible remaining errors from multiple sequence alignments using TAPER (https://github.com/chaoszhang/TAPER).
~/julia-1.7.0/bin/julia ~/TAPER/correction_multi.jl -m N -a N ${gene}_trimal_preliminary_strict.fasta > ${gene}_taper_preliminary_strict.fasta

#Save results from TAPER.
cp ${gene}_taper_preliminary_strict.fasta ../results_taper_preliminary_strict/
...
cp ${gene}_iqtree_preliminary_strict.* ../results_iqtree_preliminary_strict/
