...
~/catsequences/catsequences list.genes.to.keep
...
#Remove any alignment files with zero size, if there are any (this is only the case when use preliminary results that have not been checked by treeshrink yet).
#These files will not be present when using final gene trees!
find results_IQ-TREE_supermatrix/input_multiple_sequence_alignments/ -size 0 -print -delete

#Move the results to a directory with a sensible name.
mv allseqs.fas results_IQ-TREE_supermatrix/output_concatenated_sequences/
mv allseqs.partitions.txt results_IQ-TREE_supermatrix/output_concatenated_sequences/
mv list* results_IQ-TREE_supermatrix/output_concatenated_sequences/

#Infer a concatenation-based species tree with 1000 ultrafast bootstrap and an edge-linked partition model.
~/iqtree-2.1.3-Linux/bin/iqtree2 -s results_IQ-TREE_supermatrix/output_concatenated_sequences/allseqs.fas -o "S1321" -m GTR+F+R --prefix results_IQ-TREE_supermatrix/output_iqtree/ -bb 1000 -wbtl -nt 46 #-wbtl makes sure to write bootstrap trees including branch lengths.
...
mv results_IQ-TREE_supermatrix/output_iqtree/.iqtree results_IQ-TREE_supermatrix/output_iqtree/iqtree_report.txt
mv results_IQ-TREE_supermatrix/output_iqtree/.treefile results_IQ-TREE_supermatrix/output_iqtree/iqtree_ML_tree.tree
mv results_IQ-TREE_supermatrix/output_iqtree/.mldist results_IQ-TREE_supermatrix/output_iqtree/iqtree_ML_distances.txt
mv results_IQ-TREE_supermatrix/output_iqtree/.splits.nex results_IQ-TREE_supermatrix/output_iqtree/iqtree_bootstrap_support_values.nex
mv results_IQ-TREE_supermatrix/output_iqtree/.contree results_IQ-TREE_supermatrix/output_iqtree/iqtree_bootstrap_concensus_tree.tree
mv results_IQ-TREE_supermatrix/output_iqtree/.ufboot results_IQ-TREE_supermatrix/output_iqtree/iqtree_bootstrap_bootstrapped_trees.tree
mv results_IQ-TREE_supermatrix/output_iqtree/.log results_IQ-TREE_supermatrix/output_iqtree/iqtree_bootstrap_logfile.log

#IQ-TREE writes out hidden files, so let's make all remaining hidden files visible.
find results_IQ-TREE_supermatrix/output_iqtree/ -type f -name '.*' -execdir sh -c 'mv -i "$0" "./${0#./.}"' {} \;
...
cp results_ASTRALIII_inclusive/results_iqtree_final_inclusive/*.treefile results_IQ-TREE_supermatrix/input_tree_files/

#Create a list of gene tree files.
ls results_IQ-TREE_supermatrix/input_tree_files/*.treefile > list.all.gene.trees
...
mv list* results_IQ-TREE_supermatrix/input_tree_files/

#Concatenate all treefiles into a single input file for IQ-TREE.
cat results_IQ-TREE_supermatrix/input_tree_files/*.treefile > results_IQ-TREE_supermatrix/input_tree_files/gene_trees_combined.tree
...
~/iqtree-2.1.3-Linux/bin/iqtree2 -t results_IQ-TREE_supermatrix/output_iqtree/iqtree_ML_tree.tree --gcf results_IQ-TREE_supermatrix/input_tree_files/gene_trees_combined.tree -p results_IQ-TREE_supermatrix/input_multiple_sequence_alignments/ --scf 1000 --prefix results_IQ-TREE_supermatrix/output_iqtree_condordance_factors/ -nt 1

#Like before, IQ-TREE writes out hidden files. Let's rename these with sensible names and make them visible.
mv results_IQ-TREE_supermatrix/output_iqtree_condordance_factors/.cf.tree results_IQ-TREE_supermatrix/output_iqtree_condordance_factors/iqtree_ML_tree_with_concordance_factors.tree
mv results_IQ-TREE_supermatrix/output_iqtree_condordance_factors/.cf.tree.nex results_IQ-TREE_supermatrix/output_iqtree_condordance_factors/iqtree_ML_tree_with_concordance_factors.nex
mv results_IQ-TREE_supermatrix/output_iqtree_condordance_factors/.cf.branch results_IQ-TREE_supermatrix/output_iqtree_condordance_factors/iqtree_ML_tree_with_branch_IDs.tree
mv results_IQ-TREE_supermatrix/output_iqtree_condordance_factors/.cf.stat results_IQ-TREE_supermatrix/output_iqtree_condordance_factors/iqtree_ML_tree_concordance_factors_per_branch.stat

#IQ-TREE writes out hidden files, so let's make all remaining hidden files visible.
find results_IQ-TREE_supermatrix/output_iqtree_condordance_factors/ -type f -name '.*' -execdir sh -c 'mv -i "$0" "./${0#./.}"' {} \;

#Save a copy of the annotated supermatrix gene tree for publication.
cp results_IQ-TREE_supermatrix/output_iqtree_condordance_factors/iqtree_ML_tree_with_concordance_factors.tree 9b.IQ-TREE_supermatrix_with_concordance_factors.tree
...
#sudo treePL 10c.treePL_config > results_IQ-TREE_supermatrix/output_treePL/10d.treePL_calibrated_phylogeny.log
